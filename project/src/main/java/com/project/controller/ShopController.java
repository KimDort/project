...
public class ShopController {
  ...
  public void getBuyOrder(HttpServletRequest request, Model model, HttpSession session){
	  String[] tno=null;
	  ...
	  try {
		  ...
		  if(request.getSession().getAttribute("tno")!=null ||request.getSession().getAttribute("orderStock")!=null){
			  Object[] tno1=(Object[]) request.getSession().getAttribute("tno");
			  Object[] orderStock1=(Object[]) request.getSession().getAttribute("orderStock");
			  sno=(String) request.getSession().getAttribute("sno");
			  orderStock=(String[]) orderStock1;
			  tno=(String[]) tno1;
  ...
  public void orderComple(ShopOderVO vo,HttpServletRequest request, Model model, HttpSession session)throws Exception{
	  ...
	  if(request.getParameterValues("tno")!=null){		  
		  String[] tno=request.getParameterValues("tno");		  
		  ...
		  for(int i=0;i<request.getParameterValues("tno").length;i++){
			  ...
			  if(method.charAt(0)=='S'){
				  scvo.setProcessing('C');
			  }
			  scvo.setSno(Integer.parseInt(sno));
			  scvo.setOrderer(Integer.parseInt(mno));
			  scvo.setBank(bank);
			  scvo.setBankname(bankname);
			  scvo.setMethod(method.charAt(0));
			  scvo.setTno(Integer.parseInt(tno[i].toString()));
			  scvo.setUsepoint(0);
			  scvo.setCost(Integer.parseInt(cost[i].toString()));
			  scvo.setOrderstock(Integer.parseInt(orderstock[i]));
			  scvo.setTotalcost(Integer.parseInt(cost[i].toString())*Integer.parseInt(orderstock[i].toString()));
			  scvo.setOrdernum(Tool.getToday()+String.valueOf(tno[0])+String.valueOf(sno));
			  scvo.setZipcode(zipcode);
			  scvo.setAddress1(address1);
			  scvo.setAddress2(address2);
			  scvo.setPhone(phone);
			  scvo.setOrderdate(Tool.getTodayMinus());
			  
			  System.out.println("sno: "+scvo.getSno()+", mno:"+scvo.getOrderer()+", bank : "+scvo.getBank()+", bankname :"+scvo.getBankname()
			  +", method : "+scvo.getMethod()+", tno : "+scvo.getTno()+", usepoint : "+scvo.getUsepoint()+", cost : "+scvo.getCost()+
			  ", orderStock : "+ scvo.getOrderstock() + ", totalcost : "+ scvo.getTotalcost() +", ordernum : "+scvo.getOrdernum()+
			  ", zipcode : "+scvo.getZipcode()+", address1 : "+ scvo.getAddress1() + ", address2 : "+scvo.getAddress2()+
			  ", phone : "+scvo.getPhone());
			  orderList.add(i,scvo);
			  thingList.add(i,thService.read(Integer.parseInt(tno[i])));
			  soService.register(scvo);			  
		  }
